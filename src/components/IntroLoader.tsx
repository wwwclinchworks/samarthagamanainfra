...
export function IntroLoader({ onDone }: { onDone: () => void }) {
  ...
  useEffect(() => {
    ...
    const finish = () => {
      ...
      void import("../lib/motion").then(({ revealHero }) => revealHero())
    ...
    const runtimePromise = Promise.all([import("three"), import("gsap")])
    ...
    const start = async () => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        points.push({ x, y, z, tx: x, ty: y, tz: z, seed: Math.random() * Math.PI * 2 })
        positions[i * 3] = x
        positions[i * 3 + 1] = y
        positions[i * 3 + 2] = z
      }
      ...
      const whiteColor = new THREE.Color(cssColor("--ink", "#EDEAE1"))
      const brassColor = new THREE.Color(cssColor("--brass-bright", "#E0B472"))
      ...
      const sampleGlyph = async () => {
        ...
        const scale = Math.min((window.innerWidth / Math.max(1, window.innerHeight)) * 300, 690)
        for (let y = 0; y < off.height; y += step) {
          for (let x = 0; x < off.width; x += step) {
            if (data[(y * off.width + x) * 4 + 3] > 150) {
              result.push([((x - off.width / 2) / off.width) * scale, ((off.height / 2 - y) / off.height) * 520])
            }
          }
        }
        return result
      ...
      const draw = (time: number) => {
        ...
        cloud.rotation.x = Math.sin(seconds * 0.25) * 0.045 * drift

        for (let i = 0; i < count; i++) {
          const p = points[i]
          const noiseX = Math.sin(seconds * 0.75 + p.seed) * (2.2 * drift)
          const noiseY = Math.cos(seconds * 0.9 + p.seed * 1.7) * (1.8 * drift)
          const noiseZ = Math.sin(seconds * 0.6 + p.seed * 0.8) * (1.5 * drift)
