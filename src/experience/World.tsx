...
import { approach, brand, projectsCopy, verticals, whyChoose } from "../data/content"
    ...
    <>
      ...
      <Html position={[0, 6.4, 4.2]} center transform occlude={false} distanceFactor={18} zIndexRange={[40, 0]}>
        <div className="pointer-events-none w-[min(92vw,920px)] text-center">
          <p className="font-display text-[10px] tracking-[0.55em] text-[#6b4e1f] uppercase">{brand.tagline}</p>
      ...
      <Html position={[0, 4.8, -6]} center transform distanceFactor={16} zIndexRange={[38, 0]}>
        <div className="html3d html3d--wide">
          <p className="html3d__kicker">Anantapur</p>
          <p className="html3d__lead">Building spaces. Creating progress — from Housing Board Colony, Anantapur.</p>
        </div>
      </Html>
      ...
      {whyChoose.points.slice(0, 3).map((s, i) => (
        <Html key={s.title} position={[(i - 1) * 10, 3.2, -26]} center distanceFactor={14} zIndexRange={[34, 0]}>
          <div className="html3d html3d--stat">
            <p className="html3d__num">{String(i + 1).padStart(2, "0")}</p>
            <span>{s.title}</span>
          </div>
        </Html>
      ))}

      {approach.slice(0, 3).map((d, i) => (
        <Html key={d.t} position={[(i - 1) * 9, 2.6, -36]} center distanceFactor={13} zIndexRange={[32, 0]}>
          <a className="html3d" href="/process">
            <p className="html3d__kicker">{d.n}</p>
            <strong>{d.t}</strong>
            <span>{d.d}</span>
          </a>
        </Html>
      ...
      <Html position={[0, 3.4, -48]} center distanceFactor={16} zIndexRange={[30, 0]}>
        <div className="html3d html3d--dark">
          <p className="html3d__kicker">Projects</p>
          <strong>{projectsCopy.statusTitle}</strong>
          <a className="html3d__btn" href={waLink("project information")} target="_blank" rel="noreferrer">
            Open WhatsApp
          </a>
        </div>
      </Html>
